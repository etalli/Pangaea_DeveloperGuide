...
- [Pangaea 開発者向けガイド](#pangaea-開発者向けガイド)
  - [はじめに](#はじめに)
  - [コンセプト](#コンセプト)
  - [具体的にどんなことができるのか?](#具体的にどんなことができるのか)
  - [Pangaea仕様](#pangaea仕様)
  - [できることの例](#できることの例)
  - [Example 1： 標準の5キーから4キーに数を減らした Thumb モジュールを作る](#example-1-標準の5キーから4キーに数を減らした-thumb-モジュールを作る)
    - [完成のイメージ](#完成のイメージ)
    - [修正方針](#修正方針)
    - [KiCADでの修正](#kicadでの修正)
    - [PCBの発注](#pcbの発注)
  ...
  - [Example 2： 小指部分の列が少し下がった Pinkey module を作る](#example-2-小指部分の列が少し下がった-pinkey-module-を作る)
    ...
    - [修正方針](#修正方針-1)
    - [修正作業](#修正作業)
      - [スイッチの配置: Tips-Grid](#スイッチの配置-tips-grid)
      - [外形を整える: Tips-Edge cuts](#外形を整える-tips-edge-cuts)
      - [配線: tips: auto router](#配線-tips-auto-router)
    - [PCB発注](#pcb発注)
    - [組み立て](#組み立て-1)
    - [ソフトウェア](#ソフトウェア-1)
    - [完成](#完成-1)
...
修正のステップは、PCB上でスイッチを好きな位置に配置する、PCBの外形を整える、PCBでの配線をする、の3つです。
例1との違いは、スイッチの配置が変わり、配線がやり直しになることです。本来は、Pangaea RIのPinkey用の回路図から該当部分を持ってくるのですが、手間を省くために Pinkey のテンプレートを用意しました。

### 修正作業

スイッチの個数が8個のままでしたら、回路図の修正は不要です。PCBの修正に進みます。
...
自動配線の auto routerは、KiCAD の Plugin として Menu から実行できるようになっています。まずは freerouter をPluginからインストールします。KiCADでは以前はコマンドラインから個別に実行する必要がありましたが、最近のversionでは簡単にmenuから利用できるようになっています。
...
おそらく多くの方は、QMKのレイヤの機能を利用して、レイアのキーを押しながら上下左右に割り振ったキーを押す、という使い方を利用されていると思います。私もそうなのですが。しかし、やはりカーソルキーは独立したものがあると操作が簡単になり、かつリラックスして操作できる利点があります。
...
標準の親指モジュールは5つのキーがありますが、実は合計8個まで利用できる仕組みになっています。一番右のキーと、この3キーを利用して、合計4キーを矢印キーに割り当てることにします。
...
作る順番のおすすめとしては、まずPCBを作り、その外形をcopyして top plate, bottom plateの順に作るとよいと思います。もうこれで自分の好みに合わせたthumb moduleが作れるわけです。
...
標準のスタンドオフやネジをそのまま使います。メインモジュールからのSHコネクタを接続して組み立ては完了です。そして、ネジを少し緩めて、ご自身の指に合うように位置を調整します。
